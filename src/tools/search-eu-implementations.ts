/**
 * search_eu_implementations — Search EU directives/regulations that have Cameroonian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_cameroonian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  cameroonian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as cameroonian_statute_count
  ...
  if (input.has_cameroonian_implementation) {
    sql += ' HAVING cameroonian_statute_count > 0';
  }
