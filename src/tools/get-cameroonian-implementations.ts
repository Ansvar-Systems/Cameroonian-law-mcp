/**
 * get_cameroonian_implementations — Find Cameroonian statutes implementing a specific EU directive/regulation
...
export interface GetCameroonianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CameroonianImplementationResult {
  document_id: string;
...
export async function getCameroonianImplementations(
  db: InstanceType<typeof Database>,
  input: GetCameroonianImplementationsInput,
): Promise<ToolResponse<CameroonianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CameroonianImplementationResult[];
