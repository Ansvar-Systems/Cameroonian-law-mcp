#!/usr/bin/env tsx
/**
 * Database builder for Cameroon Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Cameroon Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Cameroonian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'CM');
    insertMeta.run('source', 'droitcamerounais.info / africa-laws.org');
