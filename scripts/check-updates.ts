#!/usr/bin/env tsx
/**
 * Cameroonian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://minjustice.gov.cm';
const PORTAL_NAME = 'Cameroon Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/cameroonian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Cameroonian Law MCP — Data Freshness Check');
