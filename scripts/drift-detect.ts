#!/usr/bin/env tsx
/**
 * Drift detection for Cameroon Law MCP.
 *
 * Checks if upstream Cameroon Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Cameroon Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'cameroonian-law-mcp/1.0 drift-detect' },
      });
